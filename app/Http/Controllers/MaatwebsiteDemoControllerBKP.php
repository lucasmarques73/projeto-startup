	...
	public function importAss()
	{
		...
		if(Input::hasFile('import_file_ass')){
			...
			if(!empty($data) && $data->count()){
				...
				if (!empty($aux)) {
					foreach (array_chunk($aux,1000) as $a) {
						DB::table('aux_pessoas_sem_rel')->insert($a);
					}
				}				
			...
			if(!empty($data) && $data->count()){
				...
				if(!empty($insert)){
					foreach (array_chunk($insert,1000) as $i) {
						...
						foreach ($i as $val) {
							$x = DB::table('pessoas')->where('cpfcnpj', array_get($val,'cpfcnpj'))->value('cpfcnpj');
							if ($x == null) {
								if (array_get($val, 'cpfcnpj') != "") {
